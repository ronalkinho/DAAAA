...
public:
	...
	//Altura
	int calculaAltura() const {
		int altura = 0;
		
		alturaAux(raiz, altura, 0);
...
protected:
	static void alturaAux(Link ra, int& mejorAlt, int actAlt) {
		...
		if (ra == nullptr)
			...
			alturaAux(ra->left, mejorAlt, actAlt + 1);
			alturaAux(ra->right, mejorAlt, actAlt + 1);
		}
	}
