...
public:
	...
	//Coste logarítmico en el nº de nodos
	vector<long int> rango(int const& k1, int const& k2) {
		
		vector<long int> v = vector<long int>();

		rangoAux(v, k1, k2, raiz);
...
protected:
	...
	//Coste logaritmico en el nº de nodos
	void rangoAux(vector<long int> &v, Clave const& k1, Clave const& k2, Link a) {

		if (a == nullptr) {
			return;
		}
		else if (menor(k1, a->cv.clave)) {
			rangoAux(v, k1, k2, a->iz);
		}
		else if (menor(a->cv.clave, k1)) {
			rangoAux(v, k1, k2, a->dr);
		}
		...
		if (a->cv.clave >= k1 && a->cv.clave <= k2) {
			...
			if (a->cv.clave >= k1 && a->cv.clave < k2) {
				bool ok = false;
				anadeHijosDr(v, k2, a->dr, ok);
			}
		}
	}

	//Coste lineal en función del tamaño del subárbol
	void anadeHijosDr(vector<long int> &v, Clave const& k2, Link a, bool &ok) {

		if (a == nullptr) {
			return;
		}
		else {
			anadeHijosDr(v, k2, a->iz, ok);
