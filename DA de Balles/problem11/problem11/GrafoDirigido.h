...
public:
    ...
    // BFS desde múltiples origenes, ss
...
public:
    ...
    size_t prenum(size_t v) const { return pre.at(v); }
    
    size_t postnum(size_t v) const { return post.at(v); }
...
public:
    ...
    bool stronglyConnected(size_t v, size_t w) const {
        return _id.at(v) == _id.at(w);
    }
    
    // devuelve el id de la componente fuerte de v
    size_t id(size_t v) const { return _id.at(v); }
private:
